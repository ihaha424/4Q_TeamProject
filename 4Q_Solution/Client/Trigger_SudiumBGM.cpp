...
void Trigger_SudiumBGM::SetIsPublic(bool isPublic)
{
	_isPublic = isPublic;
}

void Trigger_SudiumBGM::SetisDynamic(bool isDynamic)
{
	_isDynamic = isDynamic;
}

void Trigger_SudiumBGM::SetHasMesh(bool hasMesh)
{
	_hasMesh = hasMesh;
}

void Trigger_SudiumBGM::SetBoxScale(Engine::Math::Vector3 boxScale)
{
	_boxScale = boxScale;
}

void Trigger_SudiumBGM::SetBoxPosition(Engine::Math::Vector3 boxPosition)
{
	_boxPosition = boxPosition;
}

void Trigger_SudiumBGM::SetIsSphere(bool isSphere)
{
	_isSphere = isSphere;
}

void Trigger_SudiumBGM::DisposeComponents()
{
	_trigger->Dispose();
	_sound->Dispose();
}

void Trigger_SudiumBGM::PreInitialize(const Engine::Modules& modules)
{
	_sound->SetPath(L"Assets/Sounds/BGM_Shinave.wav");
	auto PhysicsManager = Engine::Application::GetPhysicsManager();
	Engine::Math::Vector3 positionData = { 70.f, -2.f, 110.f };
	Engine::Math::Vector3 scaleData = { 1.f, 1.f, 1.f };
	Engine::Math::Vector3 boxScaleData = { 90, 0.f, 0.f };
